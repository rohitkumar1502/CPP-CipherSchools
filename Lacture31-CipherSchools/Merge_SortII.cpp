...
void merge(int *arr, int start, int end)
{
    ...
    int i;
    for (i = 0; i < len1; i++)
    { // this is basicaly copying all the first half of the sorted array to arr1
        arr1[i] = arr[main_arr_index];
        main_arr_index++;
    }

    for (i = 0; i < len2; i++)
    { // this is basicaly copying all the second half of the sorted array to arr2
...
void mergeSort(int *arr, int start, int end)
{
    ...
    if (start >= end)
    { // this implies there is only 1 element, we cannot split it further
        return;
    }
    int mid = (start + end) / 2;
